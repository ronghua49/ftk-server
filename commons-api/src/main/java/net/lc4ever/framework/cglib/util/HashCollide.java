/*
 * SinoNetFramework: net.lc4ever.framework.cglib.util.HashCollide.java create by q-wang on May 3, 2015 11:35:07 PM
 * VCS Tag: $Id$
 */
...
/** 
 * "中间相遇法"是生日攻击的一种变形，它不比较Hash值，而是比较链中的中间变量。
 * 这种攻击主要适用于攻击具有分组链结构的Hash方案。
 * 中间相遇攻击的基本原理为：
 * 	将消息分成两部分，
 * 		对伪造消息的第一部分从初试值开始逐步向中间阶段产生r1个变量；
 * 		对伪造消息的第二部分从Hash结果开始逐步退回中间阶段产生r2个变量。
...
public class HashCollide {
	 ...
	 * 还是2的轮回...  
	 */
	private final BigDecimal DECIMAL_ROUTE_NUMBER = new BigDecimal(INT_ROUTE_NUMBER);
	...
	private long fixDecimal(BigDecimal data) {
		// 求余数    
		BigDecimal sub = data.divideToIntegralValue(DECIMAL_ROUTE_NUMBER.multiply(DECIMAL_ROUTE_NUMBER));
...
//		}
		...
		String target = "boxedBoolean";
		HashCollide collide = new HashCollide(85, 7, 31);
		for (int i = 0; i < 100; i++) {
			String a = collide.randomString(target.length());
			String b = collide.collide(a, target.hashCode());
			System.out.printf("[%05d]\t%s\n", i, b);
			if (b.hashCode()!=target.hashCode()) {
				System.err.println("ERROR :: src = " + a);
				System.err.println("ERROR :: dst = " + b);
				System.exit(1);
			}
