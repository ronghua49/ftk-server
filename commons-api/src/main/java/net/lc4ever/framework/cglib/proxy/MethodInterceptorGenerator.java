...
class MethodInterceptorGenerator implements CallbackGenerator {
	...
	private static final Type REFLECT_UTILS = TypeUtils.parseType("net.lc4ever.framework.cglib.util.ReflectUtils");

	private static final Type METHOD_PROXY = TypeUtils.parseType("net.lc4ever.framework.cglib.proxy.MethodProxy");

	private static final Type METHOD_INTERCEPTOR = TypeUtils.parseType("net.lc4ever.framework.cglib.proxy.MethodInterceptor");
	...
	public void generateFindProxy(ClassEmitter ce, final Map<String, String> sigMap) {
		...
		EmitUtils.string_switch(e, sigMap.keySet().toArray(new String[0]), Constants.SWITCH_STYLE_HASH, callback);
		e.end_method();
	}
}
