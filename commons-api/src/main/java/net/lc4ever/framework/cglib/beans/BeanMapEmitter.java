...
class BeanMapEmitter extends ClassEmitter {
	...
	private void generatePut(Class<?> type, final Map<String, PropertyDescriptor> setters) {
		...
		e.load_arg(0); // [bean]
		e.checkcast(Type.getType(type)); // [bean] 
		e.load_arg(1); // [bean, key]
		e.checkcast(Constants.TYPE_STRING); // [bean, key]
		EmitUtils.string_switch(e, getNames(setters), Constants.SWITCH_STYLE_HASH, new ObjectSwitchCallback() {
			...
			public void processCase(Object key, Label end) {
				...
				if (pd.getReadMethod() == null) {
					e.aconst_null(); // [bean, null]
				} else {
					MethodInfo read = ReflectUtils.getMethodInfo(pd.getReadMethod());
					e.dup(); // [bean, bean]
					e.invoke(read); // [bean, old]
					e.box(read.getSignature().getReturnType());
				}
				e.swap(); // move old value behind bean // [old, bean]
				e.load_arg(2); // new value // [old, bean, value]
				MethodInfo write = ReflectUtils.getMethodInfo(pd.getWriteMethod());
				e.unbox(write.getSignature().getArgumentTypes()[0]); // [old, bean, value]
				e.invoke(write); // [old]
				e.return_value();
			}
