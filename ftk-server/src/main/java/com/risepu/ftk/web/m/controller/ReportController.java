...
public class ReportController implements ReportApi {
    ...
    public void exportDocument(HttpServletResponse response,  String organization, String channelName,  String createTime, String number, String type,String hashs)throws UnsupportedEncodingException, ParseException {
        ...
        String hql = "select new com.risepu.ftk.web.m.dto.DocumentRequest (a.name as organizationName,a.id as organizationCode,f.dictdataName as type,c.name as documentType,b.createTimestamp as time,b.number as number,b.personalUser as idCard,b.chainHash as chainHash,e.channelName as channelName) from Organization a left join ProofDocument b on a.id=b.organization left join Template c on c.id=b.template left join OrganizationUser d on d.organizationId =a.id left join Channel e on d.inviteCode = e.inviteCode left join DictionaryData f on f.code = a.code where  b.number is not null";
        if (StringUtils.isNotEmpty(organization)) {
            organization = organization.trim();
            organization = new String(organization.getBytes("ISO8859-1"), "utf-8");
            hql += " and a.name like '%" + organization + "%'";
